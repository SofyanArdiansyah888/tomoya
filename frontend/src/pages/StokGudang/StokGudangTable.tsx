...
interface StokGudangTableProps {
  inventory: MaterialStock[]
  isLoading: boolean
}

export const StokGudangTable = ({
  inventory,
  isLoading
}: StokGudangTableProps) => {
  if (isLoading) {
    return (
      <Card>
    ...
    return (
      <Card>
        <CardContent className="p-12 text-center">
          <p className="text-gray-500">Tidak ada stok gudang ditemukan</p>
        </CardContent>
      </Card>
    )
  }
                ...
                return (
                  <tr key={`${item.lokasi_id}-${item.material_id}-${index}`} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {item.material?.unit && (
                        <div className="text-sm text-gray-500">
                          Unit: {item.material.unit_gudang}
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="text-sm font-medium text-gray-900">
                        {(item.quantity !== undefined ? item.quantity.toLocaleString('id-ID') : '-')} {item.material?.unit_gudang || ''}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="text-sm text-gray-500">
                        {item.material?.min_stok_gudang || 0}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right">

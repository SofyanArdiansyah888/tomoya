import { Coffee, Heart, Plus, Utensils } from 'lucide-react'
...
export const ProductCard = ({
  ...
  // ${isOutOfStock ? 'opacity-60 cursor-not-allowed': 'hover:shadow-md hover:scale-105 cursor-pointer'}
  return (
    <Card
      className={`p-3 h-full flex flex-col transition-all duration-200 group hover:shadow-md hover:scale-105 cursor-pointer`}
    >
      {/* Image Section */}
      <div className="aspect-video bg-gradient-to-br from-gray-100 to-gray-200 rounded-lg mb-2 flex items-center justify-center relative overflow-hidden flex-shrink-0">
        {product.gambar_produk && product.gambar_produk.length > 0 ? (
          <img
            ...
            alt={product.nama}
            className={`w-full h-full object-cover rounded-lg transition-transform duration-200 ${isOutOfStock ? '' : 'group-hover:scale-110'
              }`}
          />
        ) : (
      ...
      <div className="flex flex-col flex-1 space-y-1.5 min-h-0">
        <h3 className={`font-semibold text-sm truncate uppercase transition-colors flex-shrink-0 ${isOutOfStock
            ? 'text-gray-500'
